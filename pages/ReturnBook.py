...
from firestore_utils import get_all_users, get_borrowed_books,get_user_by_email, get_book_by_title, MAX_BORROW_LIMIT
...
def run():
    ...
    if user_email:
        ...
        if not borrowed_books_list:
            ...
            return   
        book_titles = [book.to_dict()["book_title"] for book in borrowed_books_list if book.to_dict()]
        book_title = st.selectbox("Sélectionnez le livre à retourner", options=book_titles)
        
        if st.button("Retourner"):
            user = get_user_by_email(user_email)
            if user:
                borrowed_book = next((b for b in borrowed_books_list if b.get("book_title") == book_title), None)
